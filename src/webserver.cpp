...
static void handlePostMqttConfig() {
    ...
    logMessageFmt("info", "MQTT config saved");
    sendJson(200, "{\"ok\":true,\"msg\":\"Saved.\"}");
...
static void handlePostHwConfig() {
    ...
    logMessageFmt("info", "HW config saved");
    sendJson(200, "{\"ok\":true,\"msg\":\"Saved.\"}");
...
static void handlePostSensorSetup() {
    ...
    if (xSemaphoreTake(sensorSetupMutex, pdMS_TO_TICKS(1000))) {
        ...
        logMessageFmt("info", "Sensor setup updated via web");
        sensorsReinit();
    } else {
        sendJson(503, "{\"error\":\"busy\"}");
    }
...
static void handleOtaUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        if (!otaBeginOk) return;
        if (Update.end(true))
            logMessageFmt("info", "OTA upload complete: %.0f bytes", (float)upload.totalSize);
        else {
            char errBuf[128];
...
static void handleOtaResponse() {
    if (!otaBeginOk || Update.hasError())
        ...
        logMessageFmt("info", "OTA done — rebooting");
...
static void handleConfigReset() {
    ...
    logMessageFmt("warn", "Config reset — rebooting");
...
static void handlePostFeatures() {
    ...
    logMessageFmt("info", "Features saved");
    sendJson(200, "{\"ok\":true,\"msg\":\"Saved.\"}");
...
static void webServerSetup() {
    ...
    httpServer.begin();
    logMessageFmt("info", "HTTP server started on port 80");
    logMessageFmt("info", "WebSocket log on port 81");
}
